resource "aws_cognito_user_pool_client" "client" {
  ...
  dynamic "token_validity_units" {
    for_each = try(each.value.token_validity_units, {}) != {} ? [each.value.token_validity_units] : []
    content {
  ...
  dynamic "refresh_token_rotation" {
    for_each = try(each.value.refresh_token_rotation, {}) != {} ? [each.value.refresh_token_rotation] : []
    content {
...
locals {
  ...
  clients_parsed = [for e in var.clients : {
    ...
    prevent_user_existence_errors                 = coalesce(try(e.prevent_user_existence_errors, null), try(e.client_prevent_user_existence_errors, null))
